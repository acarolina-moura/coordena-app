...
export function ReviewLista({ initialModulos }: ReviewListaProps) {
    ...
    async function handleSubmit(moduloId: string, nota: number, comentario: string) {
        ...
        if (res.success) {
            setModulos(prev => prev.map(m =>
                m.id === moduloId ? { ...m, hasReview: true, review: { nota, comentario, data: new Date() } } : m
            ));
        }
        setSubmitting(null);
    }
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {modulos.map((modulo) => (
                <div key={modulo.id} className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-6">
                    <div className="flex justify-between items-start gap-4">
                        <div>
                            <p className="text-[10px] font-bold text-teal-600 uppercase tracking-widest leading-none mb-1">{modulo.cursoNome}</p>
                    ...
                    {!modulo.hasReview ? (
                        <ReviewForm
                            ...
                            setHoveredIdx={(star) => setHoveredIdx(star !== null ? { modId: modulo.id, star } : null)}
                        />
                    ) : (
                            ...
                            <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map((s) => (
                                    <Star
                                        ...
                                        className={cn(
                                            "h-4 w-4",
                                            s <= modulo.review.nota ? "fill-amber-400 text-amber-400" : "text-gray-200"
                                        )}
                                    />
                                ))}
                            </div>
                            <p className="text-sm text-gray-600 italic">"{modulo.review.comentario}"</p>
                            <p className="text-[10px] text-gray-400">Avaliado em {new Date(modulo.review.data).toLocaleDateString()}</p>
                        </div>
                    )}
                </div>
            ))}
...
function ReviewForm({ onSubmit, loading, hoveredIdx, setHoveredIdx }: {
