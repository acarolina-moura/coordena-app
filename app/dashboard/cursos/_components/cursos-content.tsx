...
function NovoCursoDialog() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setFormData({ nome: "", descricao: "", dataInicio: "", dataFim: "", cargaHoraria: "" });
...
function DetalhesDialog({ curso, onClose }: { curso: CursoComDetalhes; onClose: () => void }) {
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden gap-0 [&>button]:hidden">
        ...
        <div className="px-6 pb-6">
          <h3 className="text-sm font-bold text-gray-900 mb-3">Módulos ({curso.modulos.length})</h3>
          <div className="flex flex-col gap-2">
            {curso.modulos.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">Sem módulos atribuídos</p>
            ) : (
              curso.modulos.map((mod) => (
                ...
                >
                  <div className="flex flex-col gap-0.5 min-w-0">
                    ...
                    <span className="text-xs text-gray-400">{mod.cargaHoraria}h</span>
                  </div>
                </div>
              ))
            )}
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">{cursos.length} cursos registados</p>
        </div>
