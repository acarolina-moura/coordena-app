...
export async function FormandoDashboard({ userName, userId }: { userName: string; userId: string }) {
  ...
  const kpis = [
    { label: 'CURSOS INSCRITOS', value: stats.cursosInscritos, icon: BookOpen, bg: 'bg-teal-50', iconBg: 'bg-teal-100', iconColor: 'text-teal-500' },
    { label: 'MÓDULOS COMPLETOS', value: `${stats.modulosCompletos}/${stats.totalModulos}`, icon: Clock, bg: 'bg-blue-50', iconBg: 'bg-blue-100', iconColor: 'text-blue-500' },
    { label: 'PRÓXIMAS SESSÕES', value: stats.proximasSessoes, icon: ClipboardList, bg: 'bg-purple-50', iconBg: 'bg-purple-100', iconColor: 'text-purple-500' },
  ]
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-gray-900">{curso.nome}</h3>
              ...
              <div className="text-right shrink-0">
                <span className="text-2xl font-bold text-teal-600">{curso.progressoGeral}%</span>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/calendario" className="flex items-center gap-1 text-sm font-medium text-teal-600 hover:text-teal-700">
              ...
              return (
                <div key={s.id} className="flex items-center gap-4 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                  <div className="flex w-12 shrink-0 flex-col items-center rounded-lg bg-teal-100 py-2">
                    ...
                    <span className="text-lg font-bold leading-tight text-teal-700">
                      {data.getDate()}
                    </span>
                  </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/notas" className="flex items-center gap-1 text-sm font-medium text-teal-600 hover:text-teal-700">
              ...
              <div key={mod.id} className="flex flex-col gap-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={cn(
                    ...
                    mod.nota === null ? 'text-gray-400' : mod.nota >= 10 ? 'text-green-600' : 'text-red-500'
                  )}>
