...
export async function getCursoFormando(userId: string) {
  ...
  const modulos = curso.modulos.map((m) => {
    ...
    const media =
      notas.length > 0 ? notas.reduce((s, n) => s + n, 0) / notas.length : null;
  ...
  if (cargaHorariaTotal > 0) {
    ...
    progressoGeral =
      modulos.length > 0
        ? Math.round(
            modulos.reduce((s, m) => s + m.progresso, 0) / modulos.length,
          )
        : 0;
  }
    ...
    insc.curso.modulos.flatMap((modulo) =>
      modulo.templatesAvaliacao.flatMap((template) =>
        template.items.map((item) => {
          ...
          let status: "PENDENTE" | "ENTREGUE" | "EM_FALTA" | "ATRASADO" =
            "PENDENTE";
          if (submissao) {
            if (
              item.dataLimite &&
              submissao.dataEntrega &&
              new Date(submissao.dataEntrega) > new Date(item.dataLimite)
            ) {
              status = "ATRASADO";
            } else {
              status = "ENTREGUE";
